...
def test_trade_frequency_blocks_second_new_setup_when_one_already_exists_this_week(monkeypatch):
    """Framework rule: weekly cap of two means with one existing weekly setup, only one new qualified setup may be persisted."""
    ...
    cfg = {
        ...
        "monitoring": {"diagnostic_min_zone_score": 40.0},
        "setups": {
            "trade_frequency": {"max_trades_per_week": 2},
            "min_rr_t1": 1.5,
            "atr_daily_max_pct": 5.0,
            "atr_daily_min_pct": 1.0,
            "tiering": {
...
def test_trade_frequency_honors_framework_max_trades_per_week_config_key(monkeypatch):
    """Framework config uses max_trades_per_week; engine should honor it as the weekly cap."""
    ...
    cfg = {
        ...
        "monitoring": {"diagnostic_min_zone_score": 40.0},
        "setups": {
            "trade_frequency": {"max_trades_per_week": 1},
            "min_rr_t1": 1.5,
            "atr_daily_max_pct": 5.0,
            "atr_daily_min_pct": 1.0,
            "tiering": {
...
def test_trade_frequency_window_boundary_counts_exact_cutoff_not_just_older(monkeypatch):
    """Framework rule: weekly max-trade cap should include setups exactly at cutoff time, but not entries one second older."""
    ...
    fixed_now_ts = int(datetime(2026, 5, 22, 12, 0, 0, tzinfo=timezone.utc).timestamp())
    week_window = 7 * 24 * 3600
    week_cutoff = fixed_now_ts - week_window

    conn.executemany(
        "INSERT INTO setups(zone_id, direction, reaction_type, trigger_type, status, created_at) VALUES (?, ?, ?, ?, ?, ?)",
        [
            (90, "long", "acceptance", "reclaim", "active", week_cutoff),
            (91, "short", "rejection", "reject", "active", week_cutoff - 1),
        ],
    )
    conn.commit()
    ...
    monkeypatch.setattr(engine, "build_setup", fake_build_setup)
    monkeypatch.setattr(engine, "persist_setup", lambda *_args, **_kwargs: persisted.append(1))

    cfg = {
        "data": {"primary_venue": "hyperliquid_perp", "symbol": "BTCUSDC"},
        "monitoring": {"diagnostic_min_zone_score": 40.0},
        "setups": {
            "trade_frequency": {"max_qualified_setups_per_week": 1, "week_window_seconds": week_window},
            "min_rr_t1": 1.5,
            "atr_daily_max_pct": 5.0,
            "atr_daily_min_pct": 1.0,
            "tiering": {
    ...
    # Positive boundary assertion: exact-cutoff setup counts and blocks new setup.
    assert len(persisted) == 0
    assert result["reasons"].get("trade_frequency_cap_reached", 0) >= 1

    # Negative pair: if only the just-older setup exists, cap should NOT fire.
    ...
    result_without_exact_cutoff = engine.run_setup_engine(conn, cfg)

    # Just-older setup is outside weekly window, so at least one new setup should pass.
    assert len(persisted) == 1
    assert result_without_exact_cutoff["persisted"] == 1
