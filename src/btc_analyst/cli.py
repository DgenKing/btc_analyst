...
def zones_cmd(tier):
    ...
    df4h=pd.read_sql_query("SELECT open_time,open,high,low,close,volume FROM candles WHERE symbol='BTCUSDT' AND venue=? AND timeframe='4h' ORDER BY open_time",c,params=[v])
    dfd=pd.read_sql_query("SELECT open_time,open,high,low,close,volume FROM candles WHERE symbol='BTCUSDT' AND venue=? AND timeframe='1d' ORDER BY open_time",c,params=[v])
    if df4h.empty:
        click.echo('No data. Run seed first.')
        return
    ...
    upsert_zones(c,zs)
    rows=c.execute('SELECT id,price_low,price_high,zone_type,source,score,tier FROM zones WHERE tier=? ORDER BY score DESC LIMIT 25',(tier,)).fetchall()
    for r in rows:
        click.echo(r)
